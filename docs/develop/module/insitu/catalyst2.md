...
An example node can be found in [vistleCatalystConfig.yaml](https://github.com/vistle/vistle/lib/vistle/insitu/catalyst2/vistleCatalystConfig.yaml).
With the `catalyst/mpi_comm` node, the MPI communicator can be passed to Vistle.
Alternatively, environment variables can be used to pass initialization parameters to Vistle:

`VISTLE_INSITU_PAUSED`
: to pause the simulation until Vistle is connected
`VISTLE_INSITU_OPTIONS`
: to pass command line arguments to Vistle in single process mode

For a multi-process Vistle:
Export `MPISIZE` and MPIHOSTS/MPIHOSTFILE in an extra shell according to the simulation,
start Vistle and the [](project:mod-InSituModule) module. The module should automatically connect and create output ports for the data fields the simulation provides.
...
Set up your OpenFOAM case as and add the following function to its `controlDict`:
```yaml	
    catalyst
    {
        type        catalyst2;
...
```	
Run your simulation and connect Vistle as described above.
